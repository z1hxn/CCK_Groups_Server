...
export const createCompetitionRouter = ({ config }) => {
  ...
  router.get('/competitions/:competitionId/rounds/day/:dayCount', async (req, res) => {
    const competitionId = Number(req.params.competitionId);
  ...
  router.get('/competitions/:competitionId/registrations/confirmed', async (req, res) => {
    const competitionId = Number(req.params.competitionId);
    if (!Number.isFinite(competitionId)) {
      return res.status(400).json({ message: 'Invalid competition id' });
    }

    const result = await proxyJson(`${config.paymentApiUrl}/registration/comp/${competitionId}/confirmed?size=80`);
    if (!result.ok) {
